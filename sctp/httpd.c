...
//----------------------------------
  ...
  for (p = res; p != NULL; p = p->ai_next) {
    int option = 1;
    listenfd = socket(p->ai_family, p->ai_socktype, IPPROTO_SCTP); //ALLAGI PRTOCOL
    /*---------------Insert ----------*/
	/*-----------Set Heartbeats-------*/
    if(setsockopt(listenfd, SOL_SCTP, SCTP_PEER_ADDR_PARAMS , &heartbeat, sizeof(heartbeat)) != 0)
        perror("setsockopt");
      /*Set rto_max*/
    if(setsockopt(listenfd, SOL_SCTP, SCTP_RTOINFO , &rtoinfo, sizeof(rtoinfo)) != 0)
        perror("setsockopt");
      /*Set Signal Handler*/
    if(sigaction(SIGINT, &sig_handler, NULL) == -1)
        perror("sigaction");
      /*Set Events */
    if(setsockopt(listenfd, IPPROTO_SCTP, SCTP_EVENTS, &event, sizeof(struct sctp_event_subscribe)) < 0)
        perror("setsockopt");
	/*-------------------------------------*/
...
void respond(int n) {
  ...
  //rcvd = recv(clients[n], buf, 65535, 0); // Allagi se ->
